...
async function loadTournaments() {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        result.error ?? 'Failed to load tournaments.'
      );
    }
...
return ( <div className="min-h-screen bg-[var(--bg-soft)]"> <Header />
  ...
  <main className="mx-auto w-full max-w-[1600px] px-4 py-8 sm:px-6 lg:px-10">
    <div className="mb-8">
      ...
      <p className="mt-2 max-w-2xl text-sm text-veldt-muted sm:text-base">
        View tournaments, competition details and tournament
        performance.
      </p>
    </div>
    ...
    {loading && (
      <div className="rounded-2xl border border-veldt-border bg-white p-8 text-center shadow-sm">
        <div className="text-sm font-semibold text-veldt-muted">
          Loading tournaments...
        </div>
      </div>
    )}
    ...
    {!loading && !error && sortedTournaments.length === 0 && (
      <div className="rounded-2xl border border-veldt-border bg-white p-10 text-center shadow-sm">
        ...
        <h2 className="mt-4 text-xl font-bold text-veldt-green">
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {sortedTournaments.map((tournament) => (
          ...
          >
            ...
            <h2 className="mt-5 text-xl font-black text-veldt-green">
              {tournament.name}
            </h2>
